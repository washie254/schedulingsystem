...
<div class="container">
	<div style="padding: 6px 12px; border: 1px solid #ccc;">
	  The following are the Statistsics of schedules allocated by the administrator <br>
		QUICK LINKS:   
		<a href="#pending"><button type="button" class="btn btn-outline-success">Awaiting time schedule </button></a>
		<a href="#approved"><button type="button" class="btn btn-outline-primary">Fully Scheduled Patients</button></a>
	</div>
</div>
<br>
	...
	<div style="padding: 6px 12px; border: 1px solid #ccc;">
		...
		<table class="table table-bordered table-striped">
			<thead>
				...
				<th scope="col"><b>Allocate Time</b></th>

				</tr>
			</thead>
			<tbody>
				<?php
				$sql = "SELECT * FROM schedules WHERE status='PENDING' AND doc_id='$docid' ORDER BY date_scheduled";
				$result = mysqli_query($db, $sql);
				while($row = mysqli_fetch_array($result, MYSQLI_NUM))
				{	
					$day = $row[4]." at &nbsp;".$row[5];
...
<div class="container" id="approved">
	<div style="padding: 6px 12px; border: 1px solid #ccc;">
        ...
        <p>Patients Scheduled for <strong>TODAY </strong></p>
        <table class="table table-bordered table-striped">
			<thead>
				<tr>
				<th scope="col"><b>Schid#</b></th>
